...
async function resetAndMigrate() {
  ...
  try {
    ...
    console.log('--- STARTING TOTAL DATABASE WIPE (EXCEPT USERS) ---');

    console.log('Clearing financial tables...');
    await client.query('TRUNCATE table transactions, budgets, savings_goals, loyalty_cards, shopping_list, planned_payments CASCADE');

    console.log('Wiping all categories...');
    await client.query('DELETE FROM categories');

    console.log('--- SEEDING FRESH HIERARCHICAL CATEGORIES ---');
    for (const main of categoriesData) {
      console.log(`Broad Category: ${main.name}`);
      ...
      for (const sub of main.subs) {
        process.stdout.write(`  - Sub: ${sub.name}... `);
        await client.query(`
    ...
    console.log('\n--- FRESH START READY ---');
    console.log('All financial records deleted. Categories re-seeded.');
  } catch (err) {
